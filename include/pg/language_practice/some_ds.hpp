...
template <typename Key, typename Value, std::size_t Capacity, typename Comp = Less<Key>>
class FixedBinaryHeap {
	...
	inline void sift_up(std::size_t idx) {
		while (idx > 1) {
			...
			if (QI_LIKELY(!comp_(buf_[idx].key, buf_[parent].key)))
				break;
	...
	inline void sift_down(std::size_t idx) {
		for (;;) {
			...
			if (left > sz_)
				return;
			std::size_t best = (right <= sz_ && comp_(buf_[right].key, buf_[left].key)) ? right : left;
			if (!comp_(buf_[best].key, buf_[idx].key))
				return;
...
template <typename T, std::size_t N, typename Op = std::plus<>, T Identity = T{}>
...
class FixedLazySegmentTree {
	...
	// range query
	T query_impl(std::size_t node, std::size_t l, std::size_t r, std::size_t ql, std::size_t qr) {
		if (ql <= l && r <= qr)
			return seg_[node];
		push(node, r - l + 1);
		std::size_t mid = (l + r) >> 1;
		T res = 0;
		if (ql <= mid)
			res += query_impl(node << 1, l, mid, ql, qr);
		if (qr > mid)
			res += query_impl(node << 1 | 1, mid + 1, r, ql, qr);
		return res;
	}
