...
class OldMember {
 ...
 public:
	OldMember(const std::string& n) : name(n), membership_(FREE), current_downloads(0) {}
	void upgrade() {
		switch (membership_) {
			case FREE:
	...
	bool download() {
		if (can_download()) {
			++current_downloads;
			std::cout << name << " downloaded a file. Current downloads: " << current_downloads << '\n';
			return true;
		} else {
...
class Member {
 ...
 public:
	...
	bool can_download() const { return _role->downloads_per_day() > current_downloads; }
